...
Original file is located at
    https://colab.research.google.com/drive/1Fw2YsSCgizv6SP9NtDAGzpdmQNsHetWc
...
Evaluation for night_light:
PSNR: 21.8 dB
SSIM: 0.72
LPIPS: 0.31

Average Metrics Across All Conditions:
Average PSNR: 22.467 dB
Average SSIM: 0.75
Average LPIPS: 0.28
"""
...
conditions = ['D75', 'D65', 'D55']
psnr_values = [22.5, 23.1, 21.8]
ssim_values = [0.75, 0.78, 0.72]
lpips_values = [0.28, 0.25, 0.31]
...
# Plot PSNR
ax1.bar(x, psnr_values, bar_width, color='skyblue', label='PSNR')
ax1.axhline(y=avg_psnr, color='blue', linestyle='--', label=f'Avg PSNR: {avg_psnr:.2f}')
ax1.set_title('PSNR (dB)')
ax1.set_xticks(x)
ax1.set_xticklabels(conditions)
ax1.legend()

# Plot SSIM
ax2.bar(x, ssim_values, bar_width, color='lightgreen', label='SSIM')
ax2.axhline(y=avg_ssim, color='green', linestyle='--', label=f'Avg SSIM: {avg_ssim:.4f}')
ax2.set_title('SSIM')
ax2.set_xticks(x)
...
# Plot LPIPS
ax3.bar(x, lpips_values, bar_width, color='salmon', label='LPIPS')
ax3.axhline(y=avg_lpips, color='red', linestyle='--', label=f'Avg LPIPS: {avg_lpips:.4f}')
ax3.set_title('LPIPS')
ax3.set_xticks(x)
...
plt.show()
print("Graph saved as 'evaluation_metrics_graph.png'")

from google.colab import drive
drive.mount('/content/drive')

"""/content/drive/MyDrive/GAN/archive (2).zip
...
!ls "/content/drive/MyDrive/GAN/archive (2).zip"

!cp "/content/drive/MyDrive/dataset/archive (2).zip" .

!cp "/content/drive/MyDrive/GAN/archive(2).zip" .
!unzip archive (2).zip
